...
export class LocalAiStudioService extends LocalAiService {
  ...
  private async compileSkillCode(source: string): Promise<void> {
    if (!source.trim()) throw new Error('Skill Python 代码不能为空')
    await new Promise<void>((resolve, reject) => {
      ...
      child.on('close', code => { clearTimeout(timer); code === 0 ? resolve() : reject(new Error(stderr.trim() || 'Skill 编译失败')) })
      ...
      this.agentCoreReady = (async () => {
        ...
        if (existsSync(bundledSkills)) {
          for (const entry of readdirSync(bundledSkills, { withFileTypes: true })) {
            if (!entry.isDirectory() || !existsSync(path.join(bundledSkills, entry.name, 'skill.json'))) continue
        ...
        for (const tool of this.toolStore.list().filter(item => !item.archived && (!item.builtin || item.current.python !== `def execute(args, context):\n    return builtin(${JSON.stringify(item.key)}, args, context)\n`))) {
          ...
          await fs.writeFile(path.join(destination, 'README.md'), `从旧工具 ${tool.key} 导入。可在 Skill 编辑器中修改 index.py。`)
        }
  ...
  private async execute(
    ...
    if (["agentStart", "agentSteer", "agentCompact", "agentApprove", "agentToolsList", "agentToolSave", "agentToolToggle", "agentToolRestore", "agentToolTest"].includes(action)) {
      throw new Error("旧工具编辑入口已停用，请使用 Agent Core 和 Skills 页面");
    }
    switch (action as keyof StudioCommands) {
      ...
      case "skillsReload":
        ...
        if (this.agentCoreService!.getStatus().runningTasks) throw new Error('请先停止运行中的 Agent 任务再重新加载 Skills')
      ...
      case "skillGetContent": {
        ...
        if (!skillDir) throw new Error(`Skill 不存在: ${skillName}`)
      ...
      case "skillCompile": {
        const skillName = required(value.skillName, "Skill 名称")
        await this.ensureAgentCore()
        if (!this.skillPlatform!.get(skillName)) throw new Error(`Skill 不存在: ${skillName}`)
        await this.compileSkillCode(textValue(value.indexPy, 100000))
        return { success: true }
      }
      ...
      case "skillSave": {
        ...
        if (this.agentCoreService!.getStatus().runningTasks) throw new Error('请先停止运行中的 Agent 任务再编辑 Skill')
        const skillName = required(value.skillName, "Skill 名称")
        ...
        if (skillJson.id !== skillName) throw new Error('Skill ID 不可在编辑时修改')
