...
test('managed runtime starts a Unix executable in a path with spaces, checks readiness and stops', {skip:process.platform==='win32'},async t=>{
 ...
 const emitted=[],runtime=new LocalAiRuntime((level,message)=>emitted.push({level,message}));t.after(()=>runtime.dispose())
 ...
 assert.ok(emitted.some(entry=>entry.level==='info'&&entry.message.includes('正在加载 model.gguf')))
 assert.ok(emitted.some(entry=>entry.level==='info'&&entry.message.includes('模型已就绪')))
