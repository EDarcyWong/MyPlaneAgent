...
async function main(){try{
 ...
 await click('.agent-settings>summary');assert.equal(await js("document.querySelector('.agent-settings').open"),true)
 await screenshot('agent-settings')
 assert.equal(await js("document.querySelector('.agent-settings-panel input').value"),'agent-fixture')
 await js("(()=>{const limit=document.querySelector('select[aria-label=\"执行上限\"]');limit.value='10';limit.dispatchEvent(new Event('change',{bubbles:true}));limit.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}))})()")
 assert.equal(await js("document.querySelector('.agent-settings').open"),false)
 ...
 assert.deepEqual(collapsedExecution.state,{planOpen:false,processOpen:false,toolOpen:false,planAfterUser:true,processAfterPlan:true,singleLine:true,detailsHidden:true,toggleLeft:true,fontDifferent:true,colorDifferent:true})
 assert.match(collapsedExecution.flow,/更新任务计划.*读取代码 \/ 文本.*修改文件/)
