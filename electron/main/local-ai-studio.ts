import {compactContext,contextMessages,contextStatus,needsCompaction,assertContextFits,isContextOverflow,inferenceBudget,type ContextMessage} from './local-ai-context.js'
...
import {DEEPSEEK_CONTEXT_TOKENS,DEEPSEEK_DEFAULT_OUTPUT_TOKENS,isDeepSeek,deepseekThinking} from '../shared/local-ai-providers.js'
import {endpoint,formatModel,jsonResponse,modelFile,numeric,record,repoId,required,sseData,stableId,textValue} from './local-ai-utils.js'
import type {StudioBootstrap,StudioCommands,StudioConnection,StudioEvent,StudioLocalModel,StudioMessage,StudioModelDetails,StudioModelFile,StudioSession,StudioSessionSummary,StudioSettings,StudioSettingsInput} from '../shared/local-ai-studio.js'
import {builtinCatalog,type StudioCatalog,type StudioDiscoveryModel} from '../shared/local-ai-catalog.js'
...
export class LocalAiStudioService extends LocalAiService {
 ...
 constructor(dataRoot:string){
  ...
  if(isDeepSeek(saved.endpoint)&&this.preferences.deepseekDefaultsVersion!==1){
   if(saved.maxTokens===2048||saved.maxTokens===8192)super.saveSettings({maxTokens:DEEPSEEK_DEFAULT_OUTPUT_TOKENS})
   ...
   this.preferences.deepseekDefaultsVersion=1;writeIntegrationJson(this.preferencesFile,this.preferences)
  }
  this.catalogFile=path.join(dataRoot,'local-ai-catalog-cache.json')
  this.pythonTools=new PythonToolRuntime();this.toolStore=new AgentToolStore(path.join(dataRoot,'local-ai-tools.json'),this.pythonTools)
 ...
 studioSettings():StudioSettings{
  ...
  return {...base,source,runtimePath:textValue(p.runtimePath,2000),runtimePort:Math.round(numeric(p.runtimePort,1024,65535,8089)),contextLength:Math.round(numeric(p.contextLength,512,contextMaximum,source==='external'&&isDeepSeek(base.endpoint)?DEEPSEEK_CONTEXT_TOKENS:4096)),gpuLayers:Math.round(numeric(p.gpuLayers,-1,999,0)),threads:Math.round(numeric(p.threads,1,256,Math.max(1,Math.min(8,os.cpus().length)))),temperature:numeric(p.temperature,0,2,0.7),topP:numeric(p.topP,0.01,1,0.95),repeatPenalty:numeric(p.repeatPenalty,0.1,2,1.1),systemPrompt:textValue(p.systemPrompt,12000),theme:p.theme==='light'||p.theme==='dark'?p.theme:'system'}
 }
 private inferenceSettings():StudioSettings{
  const settings=this.studioSettings(),runtime=this.runtime.snapshot()
  const contextLength=settings.source==='managed'&&runtime.contextLength&&['running','starting'].includes(runtime.state)?Math.floor(runtime.contextLength/Math.max(1,runtime.parallel||1)):settings.contextLength
  return inferenceBudget({...settings,contextLength})
 }
 saveStudioSettings(input:unknown){
  const value=record(input) as StudioSettingsInput,previous=this.studioSettings()
  ...
  const candidate={...this.preferences};for(const key of keys)if(value[key]!==undefined)Object.assign(candidate,{[key]:value[key]})
  if(isDeepSeek(this.studioSettings().endpoint))candidate.deepseekDefaultsVersion=1
  writeIntegrationJson(this.preferencesFile,candidate);this.preferences=candidate;return this.studioSettings()
 }
 ...
 private discoveryModel(value:unknown):StudioDiscoveryModel{
  ...
  const pipelineTag=textValue(item.pipeline_tag),capabilities:string[]=[],pipelineDescriptions:Record<string,string>={'text-generation':'文本生成与对话模型','image-text-to-text':'图像与文本理解模型'}
  if(pipelineTag==='text-generation'||tags.includes('conversational'))capabilities.push('文本对话')
  if(['image-text-to-text','visual-question-answering'].includes(pipelineTag)||tags.includes('vision'))capabilities.push('视觉')
  if(tags.some(tag=>['reasoning','chain-of-thought'].includes(tag)))capabilities.push('推理')
  if(tags.some(tag=>['tool-use','function-calling','tool-calling'].includes(tag)))capabilities.push('工具调用')
  const parameters=Number(gguf.total)||Number(record(item.safetensors).total)
  return {id,author:textValue(item.author)||id.split('/')[0],likes:Number(item.likes)||0,downloads:Number(item.downloads)||0,tags,description:textValue(card.description,500)||textValue(item.description,500)||pipelineDescriptions[pipelineTag]||'查看模型卡，了解模型用途、许可与使用方式。',pipelineTag,library:textValue(item.library_name),metricsKnown:true,lastModified:textValue(item.lastModified)||textValue(item.last_modified),parameterCount:Number.isFinite(parameters)&&parameters>0?parameters:undefined,architecture:textValue(gguf.architecture)||textValue(config.model_type)||textValue(Array.isArray(config.architectures)?config.architectures[0]:undefined),capabilities,license:textValue(card.license)||tags.find(tag=>tag.startsWith('license:'))?.slice(8)||''}
 }
 private catalogKey(input:unknown){const value=record(input);return JSON.stringify([textValue(value.query,160).trim().toLowerCase(),value.format==='all'?'all':'gguf',['downloads','likes','lastModified'].includes(String(value.sort))?String(value.sort):'downloads'])}
 async catalog(input:unknown):Promise<StudioCatalog>{
  const value=record(input),key=this.catalogKey(input),cached=this.catalogCache[key]
  const fallback=():StudioCatalog=>cached?{...cached,source:'cache',error:''}:builtinCatalog(textValue(value.query,160))
  if(value.cachedOnly===true)return fallback()
  try{
   const models=await this.search(input)
   if(!models.length&&!textValue(value.query,160).trim())throw new Error('模型目录暂未返回数据')
   const result:StudioCatalog={models,source:'live',updatedAt:new Date().toISOString(),error:''}
   delete this.catalogCache[key];this.catalogCache[key]=result
   for(const old of Object.keys(this.catalogCache).slice(0,-20))delete this.catalogCache[old]
   try{writeIntegrationJson(this.catalogFile,this.catalogCache)}catch{/* Fresh results remain usable even when disk cache cannot be written. */}
   return result
  }catch(cause){return {...fallback(),error:`暂时无法连接 Hugging Face。请检查网络或系统代理后重试；受限模型需要配置 Token。${String(cause).slice(0,300)}`}}
 }
 async search(input:unknown):Promise<StudioDiscoveryModel[]>{
  const value=record(input),params=new URLSearchParams({sort:['downloads','likes','lastModified'].includes(String(value.sort))?String(value.sort):'downloads',direction:'-1',limit:'40',full:'true',config:'true'})
  const query=textValue(value.query,160).trim();if(query)params.set('search',query)
  if(value.format!=='all')params.set('filter','gguf')
  const response=await jsonResponse(await net.fetch(`https://huggingface.co/api/models?${params}`,{headers:this.hfHeaders(),signal:AbortSignal.timeout(20000)}))
  if(!Array.isArray(response))throw new Error('Hugging Face 返回的模型列表无效')
  return response.map(item=>this.discoveryModel(item)).filter(item=>item.id)
 }
 async modelDetails(id:string):Promise<StudioModelDetails>{
  const repo=repoId(id),cached=this.metadata.get(repo);if(cached&&Date.now()-cached.time<60_000)return cached.details
  const data=record(await jsonResponse(await net.fetch(`https://huggingface.co/api/models/${repo.split('/').map(encodeURIComponent).join('/')}?blobs=true`,{headers:this.hfHeaders(),signal:AbortSignal.timeout(20000)})))
  if(!Array.isArray(data.siblings))throw new Error('模型文件列表为空或无权访问')
  const files=data.siblings.map(record).map(item=>{const file=textValue(item.rfilename,800),lfs=record(item.lfs);return {file,size:Number(item.size)||Number(lfs.size)||0,type:'file',...formatModel(file),revision:textValue(data.sha),sha256:/^[a-f\d]{64}$/i.test(textValue(lfs.sha256))?textValue(lfs.sha256):undefined}}).filter(item=>item.file&&!item.file.startsWith('.')).sort((a,b)=>Number(b.format==='GGUF')-Number(a.format==='GGUF')||a.file.localeCompare(b.file))
  const contextLength=Number(record(data.gguf).context_length)||Number(record(data.config).max_position_embeddings)
  const details:StudioModelDetails={model:this.discoveryModel({...data,id:repo}),files,revision:textValue(data.sha),gated:data.gated===true||typeof data.gated==='string'&&data.gated!=='false',contextLength:Number.isFinite(contextLength)&&contextLength>0?contextLength:undefined}
  this.metadata.set(repo,{time:Date.now(),details});if(this.metadata.size>30)this.metadata.delete(this.metadata.keys().next().value!);return details
 }
 async files(id:string):Promise<StudioModelFile[]>{return (await this.modelDetails(id)).files}
 async readme(id:string,revision:string){
  const repo=repoId(id);if(!/^[a-f\d]{40,64}$/i.test(revision))throw new Error('无法确定模型卡版本')
  const response=await net.fetch(`https://huggingface.co/${repo.split('/').map(encodeURIComponent).join('/')}/raw/${revision}/README.md`,{headers:{...this.hfHeaders(),Accept:'text/plain'},signal:AbortSignal.timeout(15000)})
  if(response.status===404)return ''
  if(!response.ok)throw new Error(`模型卡读取失败：HTTP ${response.status}`)
  if(!response.body)return ''
  const reader=response.body.getReader(),decoder=new TextDecoder();let content='',truncated=false
  try{while(true){const part=await reader.read();content+=decoder.decode(part.value,{stream:!part.done});if(content.length>120000){content=content.slice(0,120000);truncated=true;break}if(part.done)break}}finally{await reader.cancel().catch(()=>{});reader.releaseLock()}
  return content.replace(/^\uFEFF?---\r?\n[\s\S]*?\r?\n---(?:\r?\n|$)/,'').trim()+(truncated?'\n\n模型卡较长，仅显示前部分内容。完整内容请在 Hugging Face 查看。':'')
 }
 async enqueue(input:unknown){
  const value=record(input),repo=repoId(value.repoId),name=modelFile(value.file),files=await this.files(repo),file=files.find(item=>item.file===name);if(!file)throw new Error('该文件已不存在，请刷新模型列表')
  const split=name.match(/^(.*)-\d{5}-of-(\d{5})\.gguf$/i)
  const selected=split?files.filter(item=>item.file.startsWith(`${split[1]}-`)&&item.file.endsWith(`-of-${split[2]}.gguf`)):[file]
  if(split&&selected.length!==Number(split[2]))throw new Error('分片模型不完整，无法下载')
  // A companion from this same repository revision enables vision at model load.
  const projectors=files.filter(item=>isVisionProjector(item.file)&&path.posix.dirname(item.file)===path.posix.dirname(name))
  if(!isVisionProjector(name)&&projectors.length===1)selected.push(projectors[0])
 ...
 importFiles(files:string[]){
  ...
  this.saveDownloads(entries);return this.models()
 }
 removeModel(input:unknown){const value=record(input),id=required(value.id,'模型 ID'),model=this.models().find(item=>item.id===id);if(!model)throw new Error('模型不存在');if(this.runtime.snapshot().modelId===id&&['running','starting','stopping'].includes(this.runtime.snapshot().state))throw new Error('请先卸载该模型');if(value.deleteFile===true&&model.exists)unlinkSync(model.localPath);this.saveDownloads(this.readDownloads().filter(item=>item.id!==id));return this.models()}
 revealModel(id:string){const model=this.models().find(item=>item.id===id);if(!model?.exists)throw new Error('模型文件不存在');shell.showItemInFolder(model.localPath)}
 private runtimeSnapshot(){return {...this.runtime.snapshot(),endpoint:this.gateway.endpoint||`http://127.0.0.1:${this.studioSettings().runtimePort}/v1`}}
 async startApiServer(){
  ...
  await this.gateway.start(this.developer.preferences().host,settings.runtimePort,this.developer.credential())
  return this.runtimeSnapshot()
 }
 async startRuntime(id:string,options:RuntimeLoadOptions={}){
  if(this.loadingRuntime||this.stoppingServer)throw new Error('运行时正在切换，请稍后重试')
  this.loadingRuntime=true
  try{
   const model=this.models().find(item=>item.id===id);if(!model)throw new Error('模型不存在')
   if(/-\d{5}-of-\d{5}\.gguf$/i.test(model.file)&&!/-00001-of-\d{5}\.gguf$/i.test(model.file))throw new Error('请加载分片模型的第一个文件（00001）')
 ...
 sessions():StudioSessionSummary[]{
  ...
  return readdirSync(this.sessionsDirectory).filter(file=>/^[a-f\d-]{36}\.json$/i.test(file)).map(file=>this.session(file.slice(0,-5))).map(({messages,...rest})=>({...rest,messageCount:messages.length})).sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt))
 }
 session(id:string):StudioSession{const file=this.sessionFile(id);if(!existsSync(file))throw new Error('会话不存在');const session=readIntegrationJson<StudioSession>(file,{} as StudioSession);if(session.id!==id||!Array.isArray(session.messages))throw new Error('会话文件格式错误');session.context={...this.chatContextStatus(session),state:session.context?.state||'ready',message:session.context?.message};return session}
 newSession(){const settings=this.studioSettings(),now=new Date().toISOString();return this.saveSession({usage:emptyTokenUsageTotals(),id:randomUUID(),title:'新对话',model:settings.source==='managed'?this.runtime.snapshot().modelName:settings.model,systemPrompt:settings.systemPrompt,messages:[],createdAt:now,updatedAt:now})}
 updateSession(input:unknown){const value=record(input),id=required(value.id,'会话 ID');if([...this.chats.values()].some(item=>item.sessionId===id))throw new Error('请先停止生成再修改会话');const session=this.session(id);if(value.projectId!==undefined){const id=textValue(value.projectId);if(id&&!this.agent.projects().some(project=>project.id===id))throw new Error('项目不存在');session.projectId=id||undefined}if(value.title!==undefined)session.title=required(value.title,'会话标题',120);if(value.model!==undefined)session.model=textValue(value.model,500);if(value.systemPrompt!==undefined)session.systemPrompt=textValue(value.systemPrompt,12000);return this.saveSession(session)}
 deleteSession(id:string){if([...this.chats.values()].some(item=>item.sessionId===id))throw new Error('请先停止生成再删除会话');unlinkSync(this.sessionFile(id))}
 stopChat(requestId:string,owner:number){const chat=this.chats.get(requestId);if(chat?.owner===owner)chat.controller.abort()}
 async startChat(input:unknown,sender:WebContents,compactOnly=false){
  const value=record(input),requestId=required(value.requestId,'请求 ID',100),session=this.session(required(value.sessionId,'会话 ID'))
  if(this.chats.has(requestId)||[...this.chats.values()].some(item=>item.owner===sender.id||item.sessionId===session.id))throw new Error('请先等待当前生成完成')
  const images=compactOnly||value.regenerate===true?[]:chatImages(value.images)
  const imageChars=[...session.messages.flatMap(message=>message.images||[]),...images].reduce((sum,image)=>sum+image.dataUrl.length,0)
  if(imageChars>maxSessionImageChars)throw new Error('当前会话的图片已达到容量上限，请新建对话')
  const model=required(value.model,'模型',500),content=textValue(value.text,100000).trim(),settings=this.inferenceSettings(),service=this.service()
  if(settings.source==='managed'&&this.runtime.snapshot().state!=='running')throw new Error('请先在我的模型中加载模型，等待运行状态变为就绪')
  if(settings.source==='managed'&&!this.runtime.snapshot().vision&&(images.length||session.messages.some(message=>message.images?.length)))throw new Error('当前模型未加载视觉组件，无法接收图片。请下载该模型配套的 mmproj GGUF 文件，放在语言模型同一目录，然后卸载并重新加载模型。')
  session.usage??=sumTokenUsage(session.messages.filter(item=>item.role==='assistant').map(item=>item.usage||readTokenUsage({usage:{completion_tokens:item.tokens}})))
  if(!compactOnly){if(value.regenerate===true){if(session.messages.at(-1)?.role==='assistant')session.messages.pop();if(session.messages.at(-1)?.role!=='user')throw new Error('没有可重新生成的消息')}else{if(!content&&!images.length)throw new Error('消息不能为空');session.messages.push({id:randomUUID(),role:'user',content,...(images.length?{images}:{}),createdAt:new Date().toISOString()});if(session.title==='新对话')session.title=content.slice(0,36)||'图片对话'}}
  if(session.checkpoint&&session.checkpoint.through>session.messages.length)delete session.checkpoint
  session.model=model;this.saveSession(session)
  const controller=new AbortController();this.chats.set(requestId,{owner:sender.id,sessionId:session.id,controller})
  const destroyed=()=>controller.abort();sender.once('destroyed',destroyed)
  const emit=(event:StudioEvent)=>{if(!sender.isDestroyed())sender.send('local-ai:studio-event',event)}
  void this.generate(session,service,settings,controller.signal,emit,requestId,compactOnly).finally(()=>{sender.removeListener('destroyed',destroyed);this.chats.delete(requestId)})
  return {started:true}
 }
 ...
 private async prepareChatContext(session:StudioSession,service:{apiFormat:'openai'|'anthropic';endpoint:string;key:string},settings:StudioSettings,signal:AbortSignal,emit:(event:StudioEvent)=>void,requestId:string,force=false,aggressive=false){
  ...
  if(force||needsCompaction(before)){
   session.context={...before,state:'compacting',message:'正在整理进度摘要…'};publish()
   try{
    const checkpoint=await compactContext({history,system,checkpoint:session.checkpoint,budget:settings,signal,force,aggressive,summarize:async(messages,maxTokens)=>{
     session.usage!.requests++;let previous:TokenUsage|undefined
     const answer=await requestAgentModel({...service,contextLength:settings.contextLength,maxTokens,localLlama:settings.source==='managed'},session.model,messages as AgentMessage[],signal,{tools:false,summary:true,onUsage:usage=>{updateTokenUsageTotals(session.usage!,previous,usage);previous=usage}})
     return answer.content||''
    }})
    signal.throwIfAborted()
    const previous=session.checkpoint,changed=previous!==checkpoint
    session.checkpoint=checkpoint;session.context={...this.chatContextStatus(session,settings),message:changed?'已压缩，可继续':force?'暂无可压缩的旧内容':undefined}
    try{this.saveSession(session)}catch(error){session.checkpoint=previous;throw error}
   }catch(error){session.context={...before,state:'error',message:signal.aborted?'压缩已停止，原记录已保留':String(error)};publish();throw error}
  }
 ...
 private async generate(session:StudioSession,service:{apiFormat:'openai'|'anthropic';endpoint:string;key:string},settings:StudioSettings,signal:AbortSignal,emit:(event:StudioEvent)=>void,requestId:string,compactOnly=false){
  ...
  try{
   ...
   try{response=await request()}catch(cause){if(signal.aborted||!isContextOverflow(cause))throw cause;const previous=session.checkpoint;messages=await this.prepareChatContext(session,service,settings,signal,emit,requestId,true,true);if(previous===session.checkpoint)throw cause;response=await request()}
   session.messages.push(answer)
   const accept=(data:unknown)=>{const value=record(data);if(value.error)throw new Error(textValue(record(value.error).message)||'模型服务返回错误');const first=record(Array.isArray(value.choices)?value.choices[0]:undefined),delta=record(first.delta??first.message);const content=textValue(delta.content,responseCharacterLimit),reasoning=textValue(delta.reasoning_content??delta.reasoning,responseCharacterLimit);answer.content+=content;answer.reasoning=(answer.reasoning||'')+reasoning;if(answer.content.length+(answer.reasoning?.length||0)>responseCharacterLimit)throw new Error('输出已达到应用单轮容量');const reported=readTokenUsage(value);if(reported){const usage=mergeTokenUsage(answer.usage,reported);updateTokenUsageTotals(session.usage!,answer.usage,usage);answer.usage=usage;answer.tokens=usage.outputTokens}if(content||reasoning||reported)emit({type:'delta',requestId,content,reasoning,...(reported?{usage:answer.usage,sessionUsage:session.usage}:{})});if(Date.now()-lastSave>2000){lastSave=Date.now();this.saveSession(session)}}
   if(response.headers.get('content-type')?.includes('text/event-stream')){if(!response.body)throw new Error('响应流为空');for await(const data of sseData(response.body,signal)){if(data==='[DONE]')break;accept(JSON.parse(data))}}else accept(await response.json())
   if(!answer.content&&!answer.reasoning)throw new Error('模型没有返回内容')
  }catch(cause){if(signal.aborted)answer.status='stopped';else{answer.status='error';error=String(cause)}}finally{
 ...
 async dispatch(action:string,input:unknown,event:IpcMainInvokeEvent):Promise<unknown>{
  ...
  switch(action as keyof StudioCommands){
   case 'agentChooseWorkspace':{
    ...
    const workspace=new AgentWorkspace(pick.filePaths[0]),token=randomUUID()
    this.agentWorkspaces.set(token,{owner:event.sender.id,path:workspace.root});this.trackAgentOwner(event.sender)
    return {path:workspace.root,token}
   }
   case 'agentModelProfile':return modelProfile(this.probeFile,this.service().endpoint,required(value.model,'模型',500))
   case 'agentStopModelProbe':this.probes.get(event.sender.id)?.abort(new Error('测试已取消'));return
   case 'agentProbeModel':{
    if(value.kind!==undefined&&!['tools','image'].includes(String(value.kind)))throw new Error('检测类型无效')
    if(this.probes.has(event.sender.id))throw new Error('模型测试正在进行')
    const controller=new AbortController(),stop=()=>controller.abort();this.probes.set(event.sender.id,controller);event.sender.once('destroyed',stop)
    try{const settings=this.inferenceSettings();return await probeModel(this.probeFile,{...this.service(),maxTokens:256,contextLength:settings.contextLength},required(value.model,'模型',500),controller.signal,value.kind==='image'?'image':'tools')}finally{this.probes.delete(event.sender.id);event.sender.removeListener('destroyed',stop)}
   }
   case 'mcpServers':return this.mcp.list()
   case 'mcpSave':return this.mcp.save(value as McpServerInput)
   case 'mcpConnect':{
    const server=this.mcp.list().find(item=>item.id===value.id);if(!server)throw new Error('MCP 服务不存在')
    const result=await dialog.showMessageBox(owner,{type:'warning',title:'连接 MCP 服务',message:'连接 '+server.name+'？',detail:server.transport==='stdio'?'将以当前用户权限启动：'+server.command+' '+(server.args||[]).join(' ')+'。此服务未被系统沙箱隔离，可能访问文件和网络。':'将连接 '+server.url+'。调用工具时参数会发送给该服务。',buttons:['取消','连接'],defaultId:0,cancelId:0})
    return result.response===1?this.mcp.connect(server.id):null
   }
   case 'mcpDisconnect':return this.mcp.disconnect(required(value.id,'服务 ID'))
   ...
   case 'agentRestore':{
    ...
    const result=await dialog.showMessageBox(owner,{type:'warning',title:'恢复文件修改',message:'恢复这些文件到本次修改前？',detail:(change.preview.changes?.map(item=>item.path)||[change.preview.path]).join('\n')+'\n本次新建的文件会删除。检测到后续修改时将拒绝恢复。',buttons:['取消','恢复'],defaultId:0,cancelId:0});if(result.response!==1)return task
    return this.agent.restore(id,eventId)
   }
   case 'agentAudit':return this.agent.audit(required(value.id,'任务 ID'))
   case 'agentEditProject':return this.agent.editProject({id:required(value.id,'项目 ID'),...(value.name!==undefined?{name:required(value.name,'项目名称',80)}:{}),...(value.pinned!==undefined?{pinned:value.pinned as boolean}:{})})
   case 'agentRevealProject':{
    const project=this.agent.projects().find(item=>item.id===required(value.id,'项目 ID'));if(!project)throw new Error('项目不存在')
    shell.showItemInFolder(new AgentWorkspace(project.workspace).root);return
   }
   case 'agentProjects':return this.agent.projects()
   case 'agentCreateProject':{
    const grant=this.agentWorkspaces.get(textValue(value.workspaceToken))
    if(!grant||grant.owner!==event.sender.id)throw new Error('请通过选择目录按钮授权项目文件夹')
    return this.agent.createProject(grant.path,required(value.name,'项目名称',80))
   }
   case 'agentTasks':return this.agent.list()
   case 'agentTask':return this.agent.get(required(value.id,'任务 ID'))
   case 'agentDelete':return this.agent.delete(required(value.id,'任务 ID'))
   case 'agentToolsList':return this.toolStore.list()
   case 'agentToolSave':return this.toolStore.save(value as unknown as AgentToolSaveInput)
   case 'agentToolToggle':return this.toolStore.toggle(required(value.id,'工具 ID'),value.enabled===true)
   case 'agentToolRestore':return this.toolStore.restore(required(value.id,'工具 ID'),Number(value.version))
   case 'agentToolTest':{
    const project=this.agent.projects().find(item=>item.id===required(value.projectId,'项目 ID'));if(!project)throw new Error('项目不存在')
    const found=this.toolStore.version(required(value.id,'工具 ID'),value.version===undefined?undefined:Number(value.version)),args=record(value.arguments)
   ...
   case 'agentStart':{
    ...
    if(sourceId&&[...this.chats.values()].some(chat=>chat.sessionId===sourceId))throw new Error('请先停止当前对话')
    const seed=sourceId&&!taskId?this.session(sourceId):undefined
    if(seed&&this.agent.list().some(task=>task.sourceSessionId===seed.id))throw new Error('此对话已转为任务，请从列表打开该任务继续')
    const images=chatImages(value.images)
    if(this.studioSettings().source==='managed'&&!this.runtime.snapshot().vision&&(images.length||seed?.messages.some(message=>message.images?.length)||taskId&&this.agent.get(taskId).events.some(event=>event.images?.length)))throw new Error('当前模型未加载视觉组件，无法接收图片')
    this.trackAgentOwner(event.sender)
     return this.agent.start({approvalMode:value.approvalMode as AgentApprovalMode|undefined,fastMode:value.fastMode!==false,tokenBudget:value.tokenBudget as number|undefined,seed,images,projectId:projectId||undefined,workspace:grant?.path,taskId:taskId||undefined,mode:value.mode as AgentMode,model:required(value.model,'模型',500),prompt:images.length?textValue(value.prompt,16000):required(value.prompt,'任务要求',16000),maxSteps:Number(value.maxSteps)},event.sender.id,task=>{if(!event.sender.isDestroyed())event.sender.send('local-ai:agent-event',task)})
   }
   ...
   case 'agentReveal':{
    const task=this.agent.get(required(value.id,'任务 ID')),relative=required(value.path,'文件路径',2000)
    if(!task.artifacts.some(item=>item.path===relative))throw new Error('此文件不属于任务产物')
    shell.showItemInFolder(new AgentWorkspace(task.workspace).resolve(relative));return
   }
   ...
   case 'modelIcon':return this.modelIcons.get(required(value.author,'模型作者',96))
   case 'readme':return this.readme(required(value.repoId,'模型仓库'),required(value.revision,'模型版本'))
   ...
   case 'chooseDirectory':{const pick=await dialog.showOpenDialog(owner,{title:'选择模型下载目录',properties:['openDirectory','createDirectory']});return pick.canceled?null:pick.filePaths[0]}
   case 'chooseRuntime':{const pick=await dialog.showOpenDialog(owner,{title:'选择完整运行包中的 llama-server（请保留配套动态库）',properties:['openFile'],...(process.platform==='win32'?{filters:[{name:'llama-server.exe',extensions:['exe']}]}:{})});if(pick.canceled)return null;const file=pick.filePaths[0];if(!this.installer.valid(file))throw new Error('请选择 llama-server.exe（macOS/Linux 为 llama-server），并确保具有执行权限，而不是 llama-cli、压缩包或源码文件');return file}
   case 'developerState':return {preferences:this.developer.preferences(),installation:this.installer.snapshot(),runtimeFound:this.installer.valid(this.studioSettings().runtimePath),platform:process.platform,arch:process.arch,lanAddresses:this.developer.addresses(),requestLogs:[...this.developer.requestLogs(),...this.gateway.requestLogs()],serverRunning:this.gateway.running}
   case 'developerSettings':{const runtime=this.runtime.snapshot();if(this.gateway.active||runtime.pid||['starting','running','stopping'].includes(runtime.state))throw new Error('请先停止 API 服务再修改服务配置');return this.developer.save(input)}
   case 'runtimeDetect':return this.installer.detect(this.studioSettings().runtimePath)
   case 'runtimePackages':return this.installer.packages()
   case 'runtimeInstall':return this.installer.install(required(value.id,'运行包 ID'))
   case 'runtimeInstallCancel':return this.installer.cancel()
   case 'developerKey':if(this.studioSettings().source!=='managed'||!this.gateway.running)throw new Error('请先启动托管 API 服务');return this.gateway.apiKey
   ...
   case 'stopRuntime':return this.stopRuntime()
   case 'loadExternal':return this.loadExternal(input)
   case 'sessions':return this.sessions()
   case 'session':return this.session(required(value.id,'会话 ID'))
   case 'newSession':return this.newSession()
   case 'updateSession':return this.updateSession(input)
   case 'deleteSession':return this.deleteSession(required(value.id,'会话 ID'))
   case 'exportSession':{const session=this.session(required(value.id,'会话 ID')),pick=await dialog.showSaveDialog(owner,{title:'导出对话',defaultPath:session.title.replace(/[<>:"/\\|?*\x00-\x1f]/g,'_')+'.md',filters:[{name:'Markdown',extensions:['md']},{name:'JSON',extensions:['json']}]});if(pick.canceled||!pick.filePath)return false;const content=pick.filePath.endsWith('.json')?JSON.stringify(session,null,2):`# ${session.title}\n\n模型：${session.model}\n\n${session.systemPrompt?'## 系统提示词\n\n'+session.systemPrompt+'\n\n':''}${session.messages.map(message=>`## ${message.role==='user'?'你':'AI'}\n\n${message.content}${(message.images||[]).map((image,index)=>`\n\n![图片 ${index+1}](${image.dataUrl})`).join('')}`).join('\n\n')}`;writeFileSync(pick.filePath,content,'utf8');return true}
   case 'chat':return this.startChat(input,event.sender)
   case 'compactSession':return this.startChat(input,event.sender,true)
   case 'stopChat':return this.stopChat(required(value.requestId,'请求 ID'),event.sender.id)
   default:throw new Error('不支持的本地 AI 操作')
  }
 }
...
export function registerLocalAiStudio(service:()=>LocalAiStudioService,dispose:()=>void|Promise<void>){
 protectedHandle('local-ai:studio',(event,action:unknown,input:unknown)=>service().dispatch(required(action,'操作',40),input,event),true)
 let closing=false,cleaned=false
 app.on('before-quit',event=>{if(cleaned)return;event.preventDefault();if(closing)return;closing=true;Promise.resolve().then(dispose).catch(error=>console.error('Local AI shutdown failed:',error)).finally(()=>{cleaned=true;app.quit()})})
}
