...
export class PythonToolRuntime {
 ...
 private terminate(error:Error){
  ...
  if(child?.pid)void stopProcessTree(child.pid).catch(()=>child.kill('SIGKILL'));else child?.kill('SIGKILL')
 }
 private start(revision:string){
  ...
  child.stdout.on('data',chunk=>{
   if(this.child!==child)return;this.stdout+=chunk.toString('utf8')
  ...
  child.stderr.on('data',chunk=>{if(this.child===child)this.stderr=(this.stderr+chunk.toString('utf8')).slice(-20000)})
 ...
 dispose(){this.terminate(new Error('Python 工具运行时已关闭'))}
}
