...
const templates:AutomationTemplate[]=[
 {id:'project.daily-review',name:'每日回顾',description:'围绕指定主题整理进展、待办和下一步建议。',instruction:'围绕关注范围整理可确认的最新进展、待办和下一步建议。给出信息依据，不要修改文件。',mode:'general',approvalMode:'ask',parameterSchema:{type:'object',properties:{focus:{type:'string',title:'关注范围'}}}},
 ...
 {id:'project.maintenance',name:'定期整理',description:'在任务独立工作目录中整理资料和结果。',instruction:'检查独立工作目录中的现有资料，围绕维护范围整理可确认的问题和结果。不要访问其他目录。',mode:'documents',approvalMode:'ask',parameterSchema:{type:'object',properties:{scope:{type:'string',title:'整理范围'}}}}
]
...
export class AutomationService {
 ...
 private validate(input:AutomationTaskInput,previous?:AutomationTask):AutomationTask{
  ...
  const workflowId=input.workflowId?text(input.workflowId,'工作流 ID',120):undefined;if(workflowId&&!this.workflows?.exists(workflowId))throw new Error('请选择已启用的有效工作流')
