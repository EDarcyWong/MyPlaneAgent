...
import {chatMessageContent} from '../../shared/local-ai-chat.js'
...
type Run={owner:number;controller:AbortController;task:StoredTask;emit:(task:AgentTask)=>void;pending?:{eventId:string;resolve:(approved:boolean)=>void}}
const now=()=>new Date().toISOString()
const modes=new Set(['chat','coding','documents','general'])
const codingTools=new Set(['load_tool_pack','git_status','git_diff','git_log','git_show','git_blame','inspect_project','code_outline','find_symbol','find_references','find_todos','dependency_report','file_info','compare_files','archive_inspect','get_diagnostics','run_test_case','process_status','http_request','image_ocr','run_test','apply_patch','set_plan','read_history','list_files','search_files','read_file','read_document','write_file','replace_text','run_command'])
const documentTools=new Set(['load_tool_pack','inspect_project','file_info','compare_files','archive_inspect','http_request','image_ocr','apply_patch','set_plan','read_history','list_files','search_files','read_file','read_document','write_file','replace_text','create_document','create_spreadsheet','run_command'])
...
export class LocalAgentService {
 ...
 private definitions(task:AgentTask,registry=this.registry(task)):ToolDefinition[]|false{
  ...
  add(core);const loaded=task.events.filter(event=>event.kind==='tool'&&event.tool==='load_tool_pack'&&typeof event.args?.pack==='string').at(-1);if(loaded)add(toolPacks[String(loaded.args!.pack)]||[]);add(task.events.slice(-12).flatMap(event=>event.kind==='tool'&&event.tool?[event.tool]:[]))
  const text=(task.title+'\n'+task.events.filter(event=>event.kind==='user').slice(-6).map(event=>event.text).join('\n')+'\n'+task.plan.map(item=>item.text).join('\n')).slice(-12000)
  for(const [pattern,pack] of toolRoutes)if(pattern.test(text))add(toolPacks[pack])
  const chosen=new Set(selected.slice(0,maximum))
  return registry.definitions().filter(definition=>{const spec=registry.get(definition.function.name),managed=['builtin','python:builtin'].includes(spec.source);return !managed||(!allowed||allowed.has(definition.function.name))&&chosen.has(definition.function.name)})
 }
 private inference(task:AgentTask,connection=this.connection()):AgentConnection{return task.mode!=='chat'&&task.fastMode!==false?{...connection,maxTokens:Math.min(connection.maxTokens,AGENT_FAST_MAX_OUTPUT_TOKENS)}:connection}
 ...
 private save(task:StoredTask){task.updatedAt=now();writeIntegrationJson(this.file(task.id),task)}
 ...
 start(input:{approvalMode?:AgentApprovalMode;fastMode?:boolean;tokenBudget?:number;seed?:StudioSession;images?:StudioImage[];projectId?:string;taskId?:string;workspace?:string;mode:AgentMode;model:string;prompt:string;maxSteps:number},owner:number,emit:(task:AgentTask)=>void){
  ...
  if(input.taskId){if(this.runs.has(input.taskId))throw new Error('任务仍在执行');task=this.load(input.taskId);if(input.projectId&&input.projectId!==task.projectId)throw new Error('不能更改已有任务所属项目，请新建任务');this.closePendingCalls(task)}
 ...
 private closePendingCalls(task:StoredTask){
  ...
  for(const call of task.messages[last].tool_calls||[])if(!answered.has(call.id))task.messages.push({role:'tool',tool_call_id:call.id,content:JSON.stringify({error:'操作中断，结果未知。请重新读取文件验证，不能假定操作成功。'})})
 }
 private async prepareContext(task:StoredTask,connection:AgentConnection,signal:AbortSignal,emit:()=>void,force=false,aggressive=false){
  const system:AgentMessage[]=[{role:'system',content:this.system(task)}],budget={...connection,overhead:this.definitions(task)}
  ...
  if(force||needsCompaction(before)){
   ...
   try{
    const checkpoint=await compactContext({history:task.messages,system,checkpoint:task.checkpoint,budget,force,aggressive,signal,summarize:async(messages,maxTokens)=>{
    ...
    task.checkpoint=checkpoint;task.context={...contextStatus(task.messages,system,checkpoint,budget),message:changed?'已压缩，可继续':force?'暂无可压缩的旧内容':undefined}
 ...
 private async execute(run:Run,workspace:AgentWorkspace,connection:AgentConnection){
  ...
  try{
   ...
   for(let step=0;step<stepLimit;step++){
    ...
    let messages=await this.prepareContext(task,connection,signal,()=>this.publish(run,false)),answer:AgentAnswer
    let streaming:AgentEvent|undefined
    const request=()=>{
     ...
     return requestAgentModel(connection,task.model,messages,signal,{tools:definitions,thinking:task.mode==='chat'?undefined:task.fastMode===false,onReasoning:text=>{if(!text)return;event().reasoning=(event().reasoning||'')+text;update()},...(task.mode==='chat'?{onContent:(text:string)=>{if(!text)return;event().text+=text;update()}}:{}),onUsage:usage=>{updateTokenUsageTotals(task.usage!,reportedUsage,usage);reportedUsage=usage;if(Date.now()-lastProgress>400){lastProgress=Date.now();this.publish(run,false)}},onProgress:progress=>{if(signal.aborted)return;Object.assign(task.modelProgress!,progress);if(Date.now()-lastProgress>400){lastProgress=Date.now();this.publish(run,false)}}})
    }
    try{answer=await request()}catch(error){
     ...
     if(error instanceof ModelOutputLimitError){
      ...
      if(estimateTokens(messages)+estimateTokens(definitions)+connection.maxTokens+32>connection.contextLength)throw error
      if(streaming){streaming.reasoning='';streaming.text=`模型输出达到 ${connection.maxTokens} Token 上限，本轮工具未执行。`;streaming.status='failed';streaming=undefined}
      task.events.push({id:randomUUID(),kind:'assistant',text:`正在缩小本轮输出并自动重试一次。`,createdAt:now()})
      task.modelProgress={phase:'waiting',characters:0,startedAt:now()};this.publish(run)
      answer=await request()
     }else if(streaming)throw error
     else if(error instanceof ModelFormatError){task.events.push({id:randomUUID(),kind:'assistant',text:'模型返回的调用格式无效，本轮未执行工具，正在纠正一次。',createdAt:now()});messages=[...messages,{role:'user',content:'上一轮响应格式无效，未执行任何工具。请返回有效的工具调用格式：唯一调用 id、function name 和 JSON 字符串 arguments；不要重复已完成的操作。'}];answer=await request()}else{if(!isContextOverflow(error))throw error;const previous=task.checkpoint;messages=await this.prepareContext(task,connection,signal,()=>this.publish(run,false),true,true);if(previous===task.checkpoint)throw error;answer=await request()}}
    ...
    for(const call of answer.tool_calls){
     ...
     try{
      ...
      const timeout=call.function.name==='run_command'||call.function.name==='run_test'?Number(args.timeoutSeconds||60)*1000+3000:spec.timeoutMs
      ...
      if(call.function.name==='load_tool_pack'){
       const pack=String(args.pack||'');if(!toolPacks[pack])throw new ToolError('INVALID_ARGUMENTS','工具包名称无效');output=JSON.stringify({loaded:pack,message:'工具包将在下一轮模型请求中生效'})
       ...
       const approved=automatic||await this.confirmation(run,event);task.status='running';event.audit!.authorization=automatic?'automatic':approved?'confirmed':'denied'
       ...
       const prepared=await boundedTool(toolSignal,timeout,s=>workspace.prepare(call.function.name,args,s));toolSignal.throwIfAborted();event.preview=prepared.preview
       const approved=automatic||await this.confirmation(run,event);toolSignal.throwIfAborted();event.audit!.authorization=automatic?'automatic':approved?'confirmed':'denied';task.status='running'
       ...
       else{event.status='running';this.publish(run);let last=0;output=await boundedTool(toolSignal,timeout,s=>prepared.execute(s,text=>{event.output=text;if(Date.now()-last>250){last=Date.now();this.publish(run,false)}}));if(prepared.artifact){task.artifacts=task.artifacts.filter(a=>a.path!==prepared.artifact!.path);task.artifacts.push({...prepared.artifact,updatedAt:now()})}}
      }
      return output
      }
      ...
      if(['run_command','run_test'].includes(call.function.name)&&event.status!=='rejected'&&JSON.parse(output).exitCode!==0){event.status='failed';event.audit!.errorCode='COMMAND_FAILED';failures++}
      else if(event.status!=='rejected')event.status='completed'
      if(['run_command','run_test'].includes(call.function.name)){event.audit!.exitCode=JSON.parse(output).exitCode??null}
     }catch(error){
      const code=signal.aborted?'CANCELLED':error instanceof ToolError?error.code:'TOOL_FAILED';event.audit!.errorCode=code;event.status=code==='PERMISSION_DENIED'?'rejected':'failed';if(code==='PERMISSION_DENIED')event.audit!.authorization='denied';failures++
      ...
      output=JSON.stringify({error:String(error),code,...(error instanceof ToolError?{details:error.details}:{}),retryable:code==='INVALID_ARGUMENTS'||code==='UNKNOWN_TOOL',note:'仅未执行的无效调用可纠正一次；不能重放结果未知的修改或命令。'})
     }
     ...
     event.output=output.slice(0,40000);task.messages.push({role:'tool',tool_call_id:call.id,content:output.slice(0,40000)});this.publish(run)
   ...
   task.facts={goal:task.events.find(event=>event.kind==='user')?.text||task.title,changedFiles:task.artifacts.map(item=>item.path),verified:task.events.filter(event=>event.kind==='tool'&&event.audit?.exitCode===0).slice(-10).map(event=>event.tool+': exit 0 ('+event.id+')'),pending:task.plan.filter(item=>item.status!=='completed').map(item=>item.text)}
