...
export const agentTools=[
 ...
 tool('run_test_case','精确运行一个测试文件或按名称筛选测试，返回退出码和截断输出；必须确认。',{target:str('工作区内测试文件相对路径'),name:str('可选测试名称或 pytest -k 表达式'),runner:{type:'string',enum:['auto','npm','pytest']},timeoutSeconds:{type:'integer',minimum:1,maximum:300}},['target']),
 ...
 tool('run_test','运行 package.json 中的 test/check/lint/build 脚本；脚本可能执行任意代码，必须确认。',{script:{type:'string',pattern:'^(test|check|lint|build)(:[a-zA-Z0-9_-]+)?$'},timeoutSeconds:{type:'integer',minimum:1,maximum:300}},['script']),
 ...
 tool('run_command','在工作目录执行命令，例如测试、构建或 git diff。所有命令需要用户确认。命令不是沙箱，勿读取密钥或执行破坏性操作。',{command:str('完整命令'),timeoutSeconds:num('1–300 秒，默认 60')},['command'])
]
