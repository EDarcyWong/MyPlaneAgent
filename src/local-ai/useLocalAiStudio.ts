...
export function useLocalAiStudio(){
 ...
 const data=ref<StudioBootstrap>(),settings=reactive<StudioSettings>({endpoint:'http://127.0.0.1:1234/v1',model:'',maxTokens:2048,hasApiKey:false,hasHfToken:false,downloadDirectory:'',source:'external',runtimePath:'',runtimePort:8089,contextLength:4096,gpuLayers:0,threads:4,temperature:0.7,topP:0.95,repeatPenalty:1.1,systemPrompt:'',theme:'system'})
 const apiKey=ref(''),hfToken=ref(''),connection=ref<StudioConnection>(),connecting=ref(false),sessions=ref<StudioSessionSummary[]>([]),session=ref<StudioSession>(),sessionFilter=ref('')
 ...
 const modelFilter=ref(''),showMissing=ref(true),enqueueBusy=ref(''),sessionBusy=ref(false)
 ...
 const serverModels=computed(()=>connection.value?.models||[])
 ...
 async function refreshModels(){if(data.value)data.value.models=await api('models')}
 async function refreshSessions(){sessions.value=await api('sessions')}
 function hydrate(next:StudioSession){composerRevision++;images.value=[];session.value=next;model.value=next.model||settings.model||serverModels.value[0]?.id||'';systemPrompt.value=next.systemPrompt;input.value='';pending.value=undefined;followBottom.value=true;void scroll(true)}
 ...
 async function saveSettings(testConnection=false){await run('save',async()=>{const payload:StudioSettingsInput={...settings,...(apiKey.value?{apiKey:apiKey.value}:{}),...(hfToken.value?{hfToken:hfToken.value}:{})};const saved=await api('settings',payload);Object.assign(settings,saved);apiKey.value='';hfToken.value='';ElMessage.success('设置已保存');connection.value=undefined;model.value=saved.model;if(testConnection)await connect()})}
 ...
 async function removeModel(item:StudioLocalModel,deleteFile=false){try{await ElMessageBox.confirm(deleteFile?`永久删除文件 ${item.file}？`:`从模型库移除 ${item.file}？原文件会保留。`,deleteFile?'删除模型文件':'移除模型记录',{type:'warning'});if(data.value)data.value.models=await api('removeModel',{id:item.id,deleteFile})}catch(cause){if(cause!=='cancel'&&cause!=='close')report(cause)}}
 async function startModel(item:StudioLocalModel){if(!settings.runtimePath){tab.value='server';ElMessage.info('可在开发者的运行时页面自动查找或安装 llama-server，再选择模型启动服务');return}await run('load',async()=>{const state=await api('startRuntime',{id:item.id});settings.source='managed';if(data.value)data.value.runtime=state;tab.value='server';connection.value=undefined})}
 ...
 async function pasteImages(event:ClipboardEvent){
  ...
  if(sending.value||sessionBusy.value||!ready.value)return
 ...
 function removeImage(index:number){if(!sending.value&&!attaching.value)images.value.splice(index,1)}
 async function send(regenerate=false){
  if(sending.value||attaching.value||sessionBusy.value||(!regenerate&&!canSend.value))return
  const text=input.value.trim(),attachments=regenerate?[]:images.value.map(image=>({name:image.name,dataUrl:image.dataUrl}))
  ...
  try{
   ...
   requestId.value=id
   if(regenerate){if(session.value.messages.at(-1)?.role==='assistant')session.value.messages.pop()}else{session.value.messages.push({id:crypto.randomUUID(),role:'user',content:text,...(attachments.length?{images:attachments}:{}),createdAt:new Date().toISOString()});input.value='';images.value=[]}
   ...
   await api('chat',{sessionId:target.id,requestId:id,text,images:attachments,model:model.value,regenerate})
  }catch(cause){requestId.value='';pending.value=undefined;report(cause);try{session.value=await api('session',{id:target.id})}catch{}if(!regenerate){input.value=text;images.value=attachments}}
 }
 ...
 onMounted(async()=>{
  try{
   ...
   data.value=await api('bootstrap');Object.assign(settings,data.value.settings);sessions.value=data.value.sessions;model.value=settings.model
