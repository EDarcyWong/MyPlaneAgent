...
<template>
 <Teleport to="body"><aside v-if="open" class="file-tooltip" :style="position" role="tooltip" @mouseenter="$emit('enter')" @mouseleave="$emit('leave')">
  <header><strong>{{preview?.path||'文件内容'}}</strong><span v-if="line">修改自第 {{line}} 行</span></header>
  <div v-if="loading" class="file-tooltip-state">正在读取文件…</div><div v-else-if="error" class="file-tooltip-state error">{{error}}</div>
  <div v-else ref="editor" class="file-tooltip-code"><div v-for="(text,index) in lines" :key="index" :data-line="index+1" :class="{changed:index+1===line}"><span>{{index+1}}</span><code>{{text||' '}}</code></div></div>
 </aside></Teleport>
</template>

<style scoped>
.file-tooltip{position:fixed;z-index:5000;display:flex;flex-direction:column;overflow:hidden;border:1px solid #35443b;border-radius:10px;background:#101713;color:#dce8e0;box-shadow:0 16px 48px #0008}.file-tooltip>header{display:flex;align-items:center;gap:10px;min-height:40px;padding:7px 12px;border-bottom:1px solid #29352f;background:#151e19}.file-tooltip>header strong{min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px}.file-tooltip>header span{color:#8ea096;font-size:10px;white-space:nowrap}.file-tooltip-code{flex:1;min-height:0;overflow:auto;padding:7px 0 14px;background:#0d1410;font:11.5px/1.65 'SFMono-Regular',Consolas,'Microsoft YaHei UI',monospace}.file-tooltip-code>div{display:grid;grid-template-columns:48px minmax(max-content,1fr);min-height:19px}.file-tooltip-code>div.changed{background:#31452f}.file-tooltip-code span{position:sticky;left:0;padding-right:9px;border-right:1px solid #26322b;background:#0d1410;color:#586b60;text-align:right}.file-tooltip-code>div.changed span{background:#31452f;color:#b8d5bd}.file-tooltip-code code{display:block;padding:0 11px;white-space:pre;color:#cdd8d1}.file-tooltip-state{display:grid;place-items:center;flex:1;color:#8ea096;font-size:12px}.file-tooltip-state.error{color:#ee8585}
</style>
