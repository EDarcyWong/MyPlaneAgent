...
const props=defineProps<{settings:StudioSettings;runtime?:StudioRuntime;models:StudioLocalModel[];connection?:StudioConnection;connecting:boolean}>()
...
const managed=computed(()=>props.settings.source==='managed')
...
const nav=[{id:'server',label:'本地服务',icon:Connection},{id:'docs',label:'API 文档与调试',icon:Document},{id:'runtime',label:'运行时',icon:Cpu},{id:'config',label:'服务设置',icon:Setting}] as const
...
async function switchSource(){await run('source',async()=>{if(serverActive.value)throw new Error('请先停止托管服务，再切换运行方式');emit('settings',await api('settings',{source:managed.value?'external':'managed'}));emit('connect')})}
...
<template>
 <section class="developer-workspace" :class="{'show-inspector':page==='server'}">
  <nav class="dev-nav" aria-label="开发者导航"><span class="dev-eyebrow">DEVELOPER</span><button v-for="item in nav" :key="item.id" :class="{active:page===item.id}" :aria-current="page===item.id?'page':undefined" @click="page=item.id"><component :is="item.icon"/>{{item.label}}<b v-if="item.id==='runtime'&&state&&!state.runtimeFound">!</b></button><div class="dev-nav-docs"><small>兼容接口</small><button @click="showRoute('models')">模型列表</button><button @click="showRoute('chat')">Chat Completions</button><button @click="showRoute('responses')">Responses</button><button @click="showRoute('embeddings')">Embeddings</button><button @click="showRoute('messages')">Messages</button></div><div class="dev-nav-bottom"><Cpu/><span>{{managed?'llama.cpp 托管':'外部模型服务'}}<small>真实状态 · 本地工作区</small></span></div></nav>
  <main class="dev-content">
   <div v-if="error" class="dev-alert" role="alert"><span>{{error}}</span><button class="icon-btn" aria-label="关闭开发者错误" @click="error=''"><Close/></button></div>
   ...
   <template v-if="page==='server'">
    ...
    <section class="dev-section dev-logs"><header><h2>开发者日志</h2><div class="dev-actions"><button class="icon-btn" aria-label="复制开发者日志" title="复制" @click="copy(logLines.join('\n'))"><CopyDocument/></button><button class="icon-btn" aria-label="导出开发者日志" title="导出" :disabled="!!busy" @click="exportLogs"><Download/></button><button class="icon-btn" aria-label="清空开发者日志" title="清空" :disabled="!!busy" @click="clearLogs"><Delete/></button></div></header><div class="dev-log-filters"><input v-model="logFilter" placeholder="筛选日志内容" aria-label="筛选开发者日志"/><select v-model="logLevel" aria-label="日志等级"><option value="all">全部</option><option value="error">错误</option><option value="warn">警告</option><option value="api">API 调试</option></select><label><input v-model="followLogs" type="checkbox"/>跟随</label></div><pre ref="logPanel" class="dev-log-console"><span v-for="(line,index) in logLines" :key="index" :class="{'log-error':/error|failed|失败|异常/i.test(line),'log-api':line.includes('[API]')}">{{line}}{{'\n'}}</span><span v-if="!logLines.length">{{managed?'运行日志将在模型启动后显示。':'外部进程日志由外部应用管理；此处仅显示 MyPlane 的 API 调试日志。'}}</span></pre></section>
   </template>
   ...
   <template v-else-if="page==='config'">
    ...
    <section class="dev-section"><header><h2>运行方式</h2><button class="dev-button" :disabled="!!busy||active" @click="switchSource">切换为{{managed?'外部服务':'MyPlane 托管'}}</button></header><p class="dev-note">{{managed?'MyPlane 直接管理 llama-server 进程，加载 GGUF 后提供本地 API。':'连接已经运行的 LM Studio、Ollama 或兼容服务。此页托管设置不会修改外部服务。'}}</p><button v-if="!managed" class="dev-inline" @click="emit('settingsPage')">配置外部地址与密钥 <ArrowRight/></button></section>
