...
const currentPlan=computed<AgentTask['plan']>(()=>{
 const event=[...currentTurnProgressEvents.value].reverse().find(item=>item.kind==='tool'&&item.tool==='set_plan'&&Array.isArray(item.args?.steps)),steps=event?.args?.steps
 if(!Array.isArray(steps))return []
 return steps.flatMap(raw=>{if(!raw||typeof raw!=='object')return [];const row=raw as Record<string,unknown>,text=typeof row.text==='string'?row.text.trim():'',status=row.status;if(!text||!['pending','running','completed'].includes(String(status)))return [];return [{text,status:status as AgentTask['plan'][number]['status']}]})
})
const currentPlanText=computed(()=>currentPlan.value.find(item=>item.status==='running')?.text||currentPlan.value.find(item=>item.status==='pending')?.text||'暂无进行中的步骤')
const completedPlanSteps=computed(()=>currentPlan.value.filter(item=>item.status==='completed').length)
...
const modelOptions=computed(()=>{
 ...
 for(const item of props.models){const value=item.instanceId||item.id;if(value&&!options.has(value))options.set(value,item.name||value)}
 if(selectedModel.value&&!options.has(selectedModel.value))options.set(selectedModel.value,selectedModel.value)
 return [...options].map(([value,label])=>({value,label}))
})
const settingsOpen=ref(false),settingsTrigger=ref<HTMLButtonElement>(),settingsTheme=ref<Record<string,string>>({})
...
const toolArgumentPreview=(event:AgentEvent)=>{
 const command=eventCommand(event);if(command)return inlinePreview(command)
 if(!event.args)return ''
 try{return inlinePreview(JSON.stringify(event.args,(key,value)=>/(?:api.?key|token|password|secret)/i.test(key)?'[已隐藏]':typeof value==='string'&&value.startsWith('data:')?`[数据 ${value.length.toLocaleString()} 字符]`:value))}catch{return inlinePreview(String(event.args))}
}
const toolProgressLabel=(event:AgentEvent)=>{const label=toolLabel(event.tool)||event.tool||'工具',args=toolArgumentPreview(event);return args?`${label}：${args}`:label}
...
async function start(submission:Submission){
 ...
 mode.value='general'
 busy.value=true;error.value='';follow.value=true;settingsOpen.value=false
   ...
   <div ref="scroller" class="agent-scroll message-scroll" :class="{'is-empty':!hasConversation}" @scroll="trackScroll">
    ...
    <div v-else-if="task" class="agent-events">
     ...
     <template v-if="task.status!=='completed'">
      ...
      <details v-if="currentPlan.length" class="agent-plan" :open="active">
       <summary aria-label="展开或收起执行计划"><strong>执行计划</strong><span class="agent-plan-current">{{currentPlanText}}</span><small>{{completedPlanSteps}} / {{currentPlan.length}}</small></summary>
       <div class="agent-plan-items"><div v-for="(item,index) in currentPlan" :key="index" :class="item.status"><Check v-if="item.status==='completed'"/><span v-else class="agent-step-number">{{index+1}}</span><span>{{item.text}}</span><small>{{item.status==='completed'?'完成':item.status==='running'?'进行中':'待处理'}}</small></div></div>
      </details>
      ...
      <div class="agent-prompt-options">
       ...
       <ElDialog v-model="settingsOpen" class="agent-settings agent-settings-dialog" modal-class="agent-settings-overlay" width="min(500px, calc(100vw - 32px))" :style="settingsTheme" :show-close="false" append-to-body align-center @closed="focusSettingsTrigger">
        ...
        <div class="agent-settings-panel">
         <section v-if="selectedMode!=='chat'" class="agent-settings-group"><header><strong>执行参数</strong><small>控制任务运行深度和累计用量</small></header><div class="agent-settings-grid"><label class="agent-setting-field"><span>执行上限</span><select v-model.number="maxSteps" :disabled="locked" aria-label="执行上限"><option :value="10">10 轮</option><option :value="20">20 轮</option><option :value="40">40 轮</option><option :value="80">80 轮</option><option :value="120">120 轮</option><option :value="0">按 Token 预算</option></select></label><label class="agent-setting-field"><span>推理策略</span><select v-model="fastMode" :disabled="locked" aria-label="推理策略"><option :value="true">快速 · 单轮最多 8192</option><option :value="false">深度 · 完整输出预算</option></select></label><label v-if="engine==='agent'" class="agent-setting-field full"><span>任务累计 Token 预算</span><input v-model.number="tokenBudget" type="number" :min="maxSteps===0?1:0" max="10000000" step="1" :disabled="locked" aria-label="任务累计 Token 预算"/><small>{{maxSteps===0?'按预算模式必须填写大于 0 的数值':'填写 0 表示不限制累计用量'}}</small></label></div></section>
         <section v-if="engine==='chat'" class="agent-settings-group"><header><strong>生成参数</strong><small>控制回答风格和输出长度</small></header><div class="agent-settings-grid"><label class="agent-setting-field full"><span>系统提示词</span><textarea v-model="ch.systemPrompt" :disabled="locked" maxlength="12000" rows="3" aria-label="系统提示词"/></label><label class="agent-setting-field"><span>最大输出 Tokens</span><input v-model.number="ch.settings.maxTokens" type="number" min="128" :max="LOCAL_AI_MAX_OUTPUT_TOKENS" step="128" :disabled="locked"/></label><label class="agent-setting-field"><span>Temperature</span><input v-model.number="ch.settings.temperature" type="number" min="0" max="2" step="any" :disabled="locked"/></label><label class="agent-setting-field"><span>Top P</span><input v-model.number="ch.settings.topP" type="number" min="0.01" max="1" step="any" :disabled="locked"/></label><label class="agent-setting-field"><span>重复惩罚</span><input v-model.number="ch.settings.repeatPenalty" type="number" min="0.1" max="2" step="any" :disabled="locked"/></label></div></section>
         <p class="agent-settings-note"><InfoFilled/> <span>{{selectedMode==='chat'?'当前为普通对话，不会执行文件或命令操作。':fastMode?'快速策略关闭深度思考，并将 Agent 单轮输出限制为 8192 Tokens。':'深度策略保留模型思考并使用完整输出预算，运行时间和费用可能增加。'}}</span></p>
         <footer><button type="button" class="agent-primary" @click="closeSettings">完成</button></footer>
        </div>
       </ElDialog>
      </div>
